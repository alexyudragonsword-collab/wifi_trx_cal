...
### 时变多普勒:冻结的信道估计会过期(**公开签名:`TDLChannel.doppler_hz`**)

- **`TDLChannel.doppler_hz`**(默认 0 = 块衰落,与 0.7.33 之前逐比特一致)。非零时
  每个抽头按 Clarke 谱随时间变化,和角散布在圆周上、相位独立,自相关趋于
  **J0(2πf_D τ)**——这是闭式,可对拍,所以这个生成器过得了"不出货记不清的数字"
  那道线(TGn 簇表过不了)。
- **签名结论:静止时逐符号 EVM 是平的,一动就倾斜。** 40 MHz / 50 ns / 30 dB,
  122 µs 数据段:静止 −0.08 dB;170 Hz(6 GHz 下约 31 km/h)包内倾斜 **+6.2 dB**;
  670 Hz(约 121 km/h)**+12.4 dB**。这直接对质 `score_views` 文档里"冻结整包"
  那句话——它和 `ce_smooth_tones=9` 一样,是**静态条件下**才免费的假设。
- **一个被实测推翻的判断**:我最初把 2[1−J0(2πf_D T)] 当成 EVM 的上界。**它不是。**
  它是**平权**的信道变化量 E|h(t)−h(0)|²/E|h|²,而 EVM 里这个误差还要被 1/|H|²
  加权——深衰落音上估计最差、除数也最大。隔离法分离出的多普勒分量比它差
  **3.2 到 11.6 dB**。与"衰落 EVM 的功率均值发散"同源。函数改名
  `frozen_estimate_change_db` 并写明不是界限。
- **J0 的实现连踩两个坑,都被测试抓到**:幂级数在快信道到达的自变量上**不收敛**;
  改用梯形积分后撞上 `np.trapz` 在 numpy 2.0 改名而手机是 1.19.5——正是本仓库
  记载过的那次事故。最终用周期均值 `mean(cos(x·sin t))`(对光滑周期函数即谱精度),
  对 scipy 实测最大误差 **3.8e-16**,且守卫检查**字节码名字表**而不是源码文本
  (第一版守卫匹配到了自己 docstring 里讨论的那两个名字)。
- `channel_study` 增第四页:逐符号 EVM 对若干多普勒,平 vs 倾斜。
- 守卫 `tests/test_doppler.py` 9 条,三处变异双向验证。

## 0.7.32 — 2026-09-11

### 模型导频集与子带位置研究(解开打孔/RU 的评分,新增第十一个分析)

- **`OFDMConfig.pilot_set`**:默认 `"standard"`,对装不下标准导频的计划**照旧拒绝**
  ——打孔与各 RU 在标准里有各自的导频表,那是手上没有的外部输入,以标准之名编一套
  比没有更糟。`"model"` 显式选用本模型的等间隔导频集:保留本带宽的导频**数量**,
  在计划所携带的音上按秩等间隔排布,并在报告处标明是模型的、不是标准的。
- **对称性是构造出来的,不是论证出来的**:第一版用一条公式排全部秩并声称"由恒等式
  自动镜像对称",**实测判它为假**——精确秩之和是 n,而镜像列之和须为 n−1,差一,
  舍入没有理由替你补上。改为显式排一半再镜像。这是本轮第四个被测量推翻的说法。
- **`tone_plan.subband(bw, scs, n_tones, centre)`**:RU 形状的计划,但**不叫 RU**。
  标准的 RU 音索引表同属未持有的外部输入;而"损伤是否在意子带落在哪"这个问题只需要
  宽度和偏移,不需要标准的栅格。跨 DC 的子带会丢那一个音。
- **`link.subband_study` + `subband_placement` 分析**(两页,两端共用)。隔离法,
  一次只开一族损伤。80 MHz / 106 音 / −40 dBm 实测:
  - **IQ 镜像是最在意位置的**:居中切片亏 **13.5 dB**,偏置几乎为零——居中切片
    含有自己的镜像,偏置切片的镜像落在空频谱上;
  - 混频器 IM2 在 DC 处再加 **2.5 dB**;**DC 失调本身不可见**,它落在永不激活的
    DC 音上,这正是标准把它置空的一大理由;
  - 信道滤波器往反方向拉,边缘亏 **5.8 dB**;逐音均衡会精确去掉静态频响,所以这
    不是滤波器的形状,是被它衰减的音上的噪声放大;
  - 相噪在整条扫描上平坦到 **0.3 dB** 内,共模损伤本该如此;
  - **最佳位置是内部的**,既不在 DC 也不在边缘。
- 守卫 `tests/test_subband.py` 10 条,四处变异双向验证(镜像构造、默认拒绝猜测、
  子带跳过 DC、隔离是否真的生效)。金标增至七案例。

## 0.7.31 — 2026-09-11

### 音计划:标准占用、段间空音与打孔(体检 P2 之二;**公开签名:`OFDMConfig.tone_plan`**)

- **`wifitrx.waveform.tone_plan`**:`TonePlan`(有符号、排序、去 DC 的音索引)+
  `contiguous` / `standard` / `punctured`。`OFDMConfig.tone_plan=None`(默认)
  仍是历史的连续块,**逐比特一致**;`"standard"` 用本数字学的真实计划,DC 间隙与
  段间空音变成真空洞;`punctured` 按 20 MHz 子信道整块移除。
- **改动比看上去小的两个原因**:标准计划与它替换的连续块**活跃音数相同**
  (40 MHz 两侧各 242),所以所有 `sqrt(n_active)` 功率归一原样不动;有了真计划,
  导频**直接落在标准音索引上**,0.7.17 的 rank 映射退化为恒等——实测 rank 映射在
  40 MHz 偏 2 音、160 MHz 偏 16 音,现在是 0。
- **跨空音平滑是那个静默缺陷**:平滑窗口按数组相邻列做卷积,而带空洞时相邻列不是
  相邻频点。`smooth_channel_estimate` 新增 `segments` 参数,只在段内平滑。实测
  (160 MHz,30 ns 确定性响应,9 音):最差单音误差不分段 **−16.5 dB**、分段
  **−30.6 dB**,受影响 24/1992 音。**连续块仍按老行为跨 DC 平滑**——它只有一音间隙,
  而冻结夹具、金标与 0.7.18 旗舰数字都建立在那个约定上,不值得为一音作废。
- **不对称计划在 `cal.rx_iq` 变响**:镜像对拟合此前用裸 `continue` 跳过无配对的音
  (更小的拟合集、更差的校正、零提示),现在直接拒绝并说明原因。
- **一条自己的空转守卫被变异抓到**:第一版"导频精确落位"测试在标准计划下无法区分
  两条路径——因为标准计划下模型活跃集**就是**标准占用集,rank 映射与直接查表结果
  相同。改成用打孔计划区分(直接查表能指名丢了哪些导频音,rank 映射只能报计数不符)。
- 守卫 `tests/test_tone_plan.py` 12 条,三处变异双向验证。`channel_study` 分析新增
  `tone_plan` 参数(两端共用注册表),金标随之更新。

## 0.7.30 — 2026-09-11

### 传播信道进链路层(体检 P2,**不进交付面**;新增第十个分析)

- **模型** `wifitrx.link.channel.TDLChannel`:指数功率时延剖面的抽头延迟线,
  抽头在采样栅格上、各自独立复高斯、总功率归一(关闭或 rms=0 时逐比特一致)。
  **住在 `link/` 下是结构保证**——分层守卫禁止 `cal`/`chain` import `link`,
  所以交付面仍是传导模式,不靠自觉。**没有照搬 TGn 簇表**:那是手上没有的外部
  规格输入,指数剖面的 rms 时延有闭式且由 `realized_rms_delay_s` 从真实抽头重算。
- **研究** `link.channel_study` + `channel_study` 分析(三页,两端共用注册表)。
  40 MHz / 256-QAM / 30 dB 带内 SNR:最优平滑跨度随相干带宽 1/(2πτ) 走,落在
  Bc/8 到 Bc/4.5;**交付默认 `ce_smooth_tones=9` 是传导模式下的选择**,rms 时延
  ≤ 50 ns 时相对最优只亏 ≤ 0.16 dB,150 ns 时亏 6.07 dB。交付数字不变。
- **三个被实测推翻的判断**(全部记进 `cairn/传播信道与估计.md`):
  ① "隔离口径是底板"——在衰落下它是"精确信道 + 迫零",而迫零在深衰落里不好,
  modem 口径反而更优(15 ns:−27.1 对 −24.9);② "该用功率域均值"——迫零穿过
  Rayleigh 音代价 1/|H|²,E[1/|H|²] **发散**,实测三组 128 实现的均值散布 8.2 dB
  且不随实现数收窄,中位数散布 0.5 dB,改报中位数;③ "长时延是信道灾难"——
  150 ns 处 9 音读 −14.0、3 音读 −20.1,6 dB 是接收机没调窗口,不是超出 CP
  (指数剖面 6τ 处只剩 0.25 % 能量,跨 CP 是斜坡不是悬崖)。
- **口径**:`snr_db` 定义为**带内**每音 SNR(平坦信道下 iso 读数 = −snr_db);
  时域口径会好 10log10(os_nfft/n_active) = 6.26 dB,差得足以误导读者,已由守卫钉死。
- 守卫 `tests/test_channel.py` 11 条,四处变异双向验证。金标增至六案例。

## 0.7.29 — 2026-09-11

### 默认分支切到 `main`,同批复核观测面与派发协议(仅文档与规则,无代码改动)

- **切换理由**:分工变干净——**push 覆盖会动的分支,cron 覆盖不动的分支**。此前
  两者都盯同一棵会动的树,cron 几乎没有独有价值;现在它对着固定的 `main`,红了
  就只能是环境变了。次要理由:fresh clone 落在 `main`,而不是一个会话名分支。
- **徽章改为四个**:`ci / main`(交付方看的主干)、`ci / nightly`(环境漂移)、
  `ci / dev`(**日常真正会变红的地方**)、`android / golden`(端上裁决)。
- **0.7.28 写的"三个徽章都不带 `branch=`"被前提变化推翻**:默认分支一切到
  `main`,`ci / dev` 与 `android / golden` 的过滤条件在 `main` 上运行数就都是
  **0 条**(实测),不钉 `branch=` 就是两个永远空白的徽章。这两个改为钉分支,
  代价是分支改名时会静默变空白,已在 README 注明必须同批更新。
- **派发协议加一条硬规则**(`AGENTS.md` 与 `cairn/Android出货线.md`):金标
  dispatch **必须显式选中被测提交所在的分支**。Run workflow 的默认 ref 现在是
  `main`,误用默认会去裁决 `main`;而当 `main` 恰好等于开发分支顶端时,这次误
  派发**在结论上完全看不出来**——版本核对、金标对拍、导出检查全会通过,只是测的
  不是你以为的那棵树。
- **两处需要照着读的过渡态**:`ci / nightly` 在切换后的第一次 cron 之前是空白的
  (`main` 上此前无定时运行),这是一次性过渡不是缺陷;`ci / main` 近乎恒绿,因为
  `main` 只从已裁决为绿的提交快进而来——它回答"主干跑过且过了",不是"主干挡住了
  什么",别把它当守卫。

## 0.7.28 — 2026-09-11

### 观测面:让会红的那条 lane 跑在有人看的地方(CI/脚本/README,无库代码改动)

- **full lane 改为也在 push 上跑**(`ci.yml` 去掉 `full` 的 `if:`)。红三周的
  前提是它只在凌晨的 cron 里跑;挂到 push 上是**消掉"只在没人看时才跑"这个
  不对称性**,不是再加一个探测器。cron 保留,职责收窄成"没有推送的日子里抓
  环境漂移"。
- **代价的实测值,以及一个被推翻的估计**:落地前按 run #169 同一次运行里两个
  job 的差(fast 3 分 34 秒 / full 4 分 05 秒)估成"关键路径只多约半分钟"。
  **这个估法是错的**——它比的是同一次运行内部的两个并行 job,而推送线的墙钟由
  最慢的那个决定,且 full 自身跨 runner 抖动。实测推送线总时长:只有 fast 时
  **3 分 26 秒**(run #170),加上 full 后 **4 分 50 秒**(run #176,full 自身
  4 分 47 秒)。**实际约 +1 分 24 秒**,不是半分钟。结论不变,数字更正。
- **徽章按 `event` 拆成三个**(push / schedule / android 手动派发)。原来那个
  不带过滤的徽章只反映最近一次运行,推送频繁且绿,于是它长绿而每次 schedule
  都红——0.7.16 加徽章正是为了让红可见,却看了错的那条 lane。**三个都不带
  `branch=`**:徽章默认跟随仓库默认分支,而 schedule 与 workflow_dispatch
  **只在默认分支上触发**;实测 `main` 上这两类运行均为 0 条,写死
  `branch=main` 会做出两个永远空白的徽章。
- **cron 失败落到 issue**(`notify` job,`failure() && event == 'schedule'`):
  已存在同名 open issue 则追评论,不重复开。仅对 cron 生效——push 的失败本来
  就在推的人面前。**这一条尚未端到端验证**,它只在真实的定时失败时触发;已记入
  专题文档的开放问题。
- **两条 lane 脚本改为逐项收集失败再统一退出**,不再第一个失败就退。此前
  `set -e` 下一条红的陈旧检查把原理图检查整个跳过了三周,而一条红的测试会同时
  遮住两者。同时把秒级的 `build_assets --check` 移到 27 秒的文档重建之前;文档
  **构建**失败时明说"staleness NOT evaluated",不让构建失败读成陈旧判定。
  变异验证:用一个必定失败的 `ruff` 桩跑 `ci_fast.sh`,先确认桩确实失败,套件
  仍跑完 322 条、脚本退出 1(旧结构下套件会被整个跳过)。
- `AGENTS.md` 删掉一句重复:双向验证那条已经给了更强的规则,其后又单独重复
  "必须用变异确认会红"读起来像把要求降回单向。
- `cairn/守卫有效性.md` 增补「流程层的第四类:观测面」与空白徽章这一新实例;
  `docs/backlog_zh.md` B27。

## 0.7.27 — 2026-09-11

### 第三篇知识专题:守卫有效性(仅知识层与规则,无代码改动)

- `cairn/守卫有效性.md`:把守卫失效归成**四种形态**——空转(什么也没断言到)、
  错靶(匹配到名字或注释而非干活的行)、恒红(前提在任何提交上都不成立)、
  被遮(本身正确但排在会失败的步骤后面),给出各自在 CI 上的样子与抓法,
  并列出本项目每一次踩中的台账(0.7.5 / 0.7.6 / 0.7.16 / 0.7.23 / 0.7.26)。
- **核心结论**:**变异验证只抓得住前两种。**恒红的守卫在变异下照样红,与
  严格的守卫从结论上完全同相;唯一能分开它们的是反过来问"没毛病时它会不会
  绿"。被遮的那一类连红都不给——`set -e` 下它在日志里与"不存在"无异。
- `AGENTS.md`「验证」节据此补一条规则:守卫**双向验证**(变异必须红、还原
  必须绿),关键检查不得排在会失败的步骤之后。原有"变异先确认注入成功"
  与"匹配干活的那一行"两条保留。
- 记录一条被推翻的判断:体检报告 P0-1 说修掉金标断言后覆盖率、文档陈旧、
  原理图三步就会真正执行——只对第一步成立,后两步直到 0.7.26 才第一次跑。
- **敞口(未决)**:夜间 schedule lane 仍无徽章与通知,其红靠人工复核发现。

## 0.7.26 — 2026-09-11

### 夜间文档陈旧检查修复:一个结构上不可能通过的守卫(仅 CI/工具,无出货代码改动)

- **症状**:`ci.yml` 的夜间全量 job 自 8 月 22 日起每一次都红,跨多个 head
  commit(#91 … #167)。0.7.16 修的是推送快线,夜间红的是另一件事,被排在
  前面的失败步骤长期遮住。
- **根因**:陈旧检查把提交在库里的 HTML 与当场重建的 HTML 做文本比对,只把
  ISO 时间戳归一化,**留下了 provenance 页脚里的 commit id**。文件不可能记录
  包含它自己的那个提交——已提交的副本是在父提交(且工作树 dirty)上生成的,
  重建则盖上 HEAD——所以两者永远不等,检查永远红。
- **后果**:排在它后面的原理图检查 `tools/build_assets.py --check` 从未执行过
  (现查:本身是干净的,4 张图最新)。覆盖率与全量慢测在 0.7.16 后其实已绿。
- **修法**:`tools/docs_staleness.py` 比对前整体去掉 provenance 页脚(commit
  id、dirty 标记、构建时间、numpy 版本都是构建情形而非内容);页脚不再匹配时
  **报错而不是放行**,以免重蹈"守卫匹配到空"的覆辙。
- **守卫的守卫**:`tests/test_docs_build.py` 新增三条钉住归一化逻辑(只差
  provenance 不算陈旧、正文差一个词算陈旧、页脚缺失或重复报错)。端到端变异
  验证:改一处教程正文并重建 → 红;还原 → 绿(变异先确认注入成功)。
...
- README 损伤表下新增"模型边界"段:所有 EVM 为传导模式数(无传播信道)、有效音为
  无空音连续块(无 OFDMA/打孔/DC 空音)、AGC 无建立动态、交付面不含 MIMO 残差;
  链路级验证走 cal-state 残差注入。此前这些只散见于教程 ch9。
- **决定不做**(记入 backlog B24):`tx_iq` 多音合并——体检把
  `calibrate_tx_iq_envdet` 的三重循环当成套件最大成本,核实后它是包络检波器
  **备用**路径、不在出货序列里、其测试 0.3 s;主路径 `calibrate_tx_iq` 本来就是
  多音梳。套件热点实为 GUI 全流程用例(15 s / 11 s),不值得为此改算法。
- 传播信道 / OFDMA·打孔·空音 / AGC 建立动态三项物理扩展待用户选定优先级。
...
- **问题(工程体检发现)**:`waveform/pilots.py` 只有一张表——20/40/80/160 MHz
  各 4/6/8/16 个导频,是 802.11a/n/ac 的位置;320 MHz 是自造的"160 复制"
  32 个。它被同时用于 11ax/be 数字学,而标准的 RU 导频集是 242 音 8 个
  (±22, ±48, ±90, ±116)、484 音 16 个、996 音 16 个、2×996 32 个、4×996 64 个;
  旧 320 MHz 集最外导频只到 ±487(有效音 ±1992),覆盖约 24 % 带宽。未列出的
  带宽还会静默退化成"8 个等间距"。
- **修法**:两张表按子载波间隔选择(78.125 kHz → 11ax/be,312.5 kHz → 传统);
  160/320 MHz 集按标准的音位复制规则(80 MHz 集置于 ±512,再置于 ±1024)生成。
  模型的有效音是无空音的连续块,所以标准导频按**序号映射**到模型音位——
  导频数与跨带杠杆臂保持标准值,每个导频最多移动其内侧空音数(2–44 音)。
  未定义的带宽/数字学直接 `ValueError`,不再猜。
  新增 `standard_pilot_tones()`;`pilot_positions()` 签名不变。
- **数字变化(40 MHz 11ax/be,单 LO,8 帧,seed 0)**:配置 ④ −42.14 → −42.38 dB,
  导频台阶 +0.33 → +0.09 dB(理论 10·log(1+1/32) = 0.13);②、③ 不变。
  制式差距(40 MHz,32 帧,4 种子):1.4 ± 0.2 → **1.10 ± 0.27 dB**——
  11ax/be 仍更差,方向不变,数值缩小一个导频项。80 MHz:N_p 8 → 16,
  差距约 1.5 → 1.0 dB。**更正**:0.7.10/0.7.11 的"1.4 dB"、0.7.15 三级台阶
  笔记的 ④−③ 读数与"按带宽的导频台阶"表都建立在错误的导频数上;三篇
  笔记已按新表重建,笔记 1 的制式差距改为脚本现算(32 帧 × 4 种子)。
- **守卫**:`tests/test_waveform.py` 钉两套表的导频数、对称性、序号映射的
  杠杆臂(≤ 5 % 带边)与"不猜"行为(变异验证:40 MHz 集少一个导频即红);
  `test_gui_specs.py` 的制式差距断言改为闭式锚定(两符号长的 ICI 底板比
  + 两导频项之差,±0.6 dB 种子散布);`test_clock_iip2.py` 跟踪器断言改为
  收敛速率(20·log(1−μ) = −8 dB/帧)而不是第 6 帧的绝对值。
- **出货代码**:`src/wifitrx/waveform/pilots.py`、`app/specs.py`(页 (d) 标题
  按各制式标 N_p)。金标重生成(24 指标 / 5 页不变,`n_pilots` 8 → 16),
  Android 金标 job 需重新 dispatch。
...
- **需求**:用户要求把 ② → ③ → ④ 三级台阶的分析(每级加哪种误差、随什么
  平均、随 Δf/LO 电平/符号长度怎么变、各自靠什么消)渲染成 PDF 归档。
- **内容(3 页)**:读数表(Δf = 0 与 2 kHz,②/③/④);两条比例公式
  $P_3/P_2\approx1+\rho/2$(两 LTF 平均,+1.76 dB 理论 / +1.73 实测)与
  $P_4/P_3\approx1+1/(2N_p)-1/(2N_{\text{active}})$(+0.35 理论 / +0.33 实测);
  三种误差的性质对照表;导频台阶随带宽的预期表(20–320 MHz:0.51–0.07 dB,
  26-tone RU 0.97 dB);图:两 Δf 下的台阶柱状图 + 两个台阶对 N_p 的理论/实测。
- **做法**:并入 `tools/build_pn_cpe_note.py`(第三个 builder),读数由脚本
  调 `_pn_nominal` 现算;产物 `docs/pn_cpe_note_estimation_ladder.{pdf,png}`。
  只动 docs/tools/README,不改随 APK 出货的代码,不重跑金标。

## 0.7.14 — 2026-09-07

### `pn_cpe_study` 残余 CFO 旋钮与第五页:跟踪的真正价值

- **起因**:用户问配置 ①"完全不去 CPE 星座为什么没糊"。答案是模型边界:帧长
  0.3 ms、合成最低频偏 fs/n ≈ 3 kHz、无 CFO、谱 10 kHz 以下平延——相位在帧内
  走不远(出货 LO 只有 0.37° rms),所以"不跟踪"只读到 ∫S_φ df。真实链路里
  跟踪的价值大头是残余 CFO 与近端漂移,而这些在相噪隔离研究里结构性为 0。
  用户要求加开关注入残余 CFO 看配置 ① 糊掉,并把这部分价值实现出来。
- **改动**:新参数 `cfo_hz`(默认 0,所有页生效);配置 ①/② 永不捕获(① 逐
  符号旋转、糊掉;② 去掉每符号旋转、符号内斜坡留成 ICI,恰为
  $1-\mathrm{sinc}^2(\Delta f\,T)$——CFO 就是一条落在 Δf 的相噪谱线);配置
  ③/④ 加入 modem 的捕获级:**LTF 对粗估 CFO + 导频共模相位对时间的斜率细估**
  (与 `cal/tracking.py` 同一回归)+ LTF 信道估计。第五页 "Residual CFO"
  扫 0–10 kHz,四配置 + 两条闭式(① 含旋转累积的 |sinc·e^{jθ_k}−1|² 平均,
  ② 含 1−sinc²);新增指标 `cfo_hz`、`cfo2k_*`、`cfo2k_tracking_value_db`。
  页 (c) 的闭式线在注入 CFO 时含 CFO 项;标题/图例按是否注入调整措辞。
- **实测(40 MHz 11ax,单 LO,8 帧,Δf = 2 kHz)**:不跟踪 **+4.1 dB**(糊掉),
  genie CPE **−26.7 dB**(闭式 −26.6),modem 形态 **−42.1 dB** = 无 CFO 时的读数
  ——跟踪价值 46 dB;10 kHz 时 genie CPE 只剩 −12.8 dB,modem 形态仍 −42.1。
  11ac 同 2 kHz:genie CPE −38.1 dB——符号短 4×,符号内斜坡 ICI 小 12 dB。
- **顺带抓出一个真机制并修掉**:只用 LTF 对粗估就去旋转整帧是不够的——相噪
  让 LTF 对读出**伪 CFO**(12.8 µs LTF 约 25 Hz rms,3.2 µs LTF 约 180 Hz rms,
  短 LTF 把同样的相位差放大 4 倍),按它去旋转把误差灌回符号内斜坡 ICI,
  11ac 的配置 ③/④ 因此差 0.6 dB、两制式差值从 1.5 缩到 0.3 dB。加导频斜率
  细估(基线是整帧,长两个量级)后 11ac 回到 −43.41 / −43.00 dB,差值恢复。
  这就是真实 modem 必须用导频环细跟 CFO 的原因,模型把它量出来了。
- **口径提醒(写进页内文字)**:此前各页的"CPE 收益"(11ax 0.36 dB)只是
  LO 谱中 0.443/T 以下那份;跟踪对 CFO/近端漂移的价值不在其中,由本页单独
  给出。两者不能相加成"跟踪总收益"——前者是相噪,后者是确定性旋转。
- **守卫**:40 MHz Δf = 2 kHz 下 ① > −10 dB、③/④ 与 Δf = 0 读数差 < 0.3 dB、
  ② 对闭式 ±0.5 dB、闭式 e₂ == (πΔfT)²/3(1%);去掉捕获级的变异已红。
  金标 `golden.json` 重生成(24 指标 / 5 页);**run #58 三 job 全绿**,解释版与
  编译版端上均 5 run / 0 failed / 0 skipped,金标按 24 指标 / 5 页通过,编译版
  wheel 打 0.7.14。

## 0.7.13 — 2026-09-06

### 去 CPE 后的自由 VCO 地板 π²k₂T/3:进库、画上 PLL 页,并归档第二篇短文

- **起因**:用户在 `pn_cpe_study` 的 PLL 页上看到,同为 40 MHz、配置 ②,11ax
  有一个明显的最佳环路带宽,而 11ac 在环路带宽小于某值后相噪 EVM 贡献基本
  不变,要求分析。
- **闭式解**:去 CPE 后自由振荡 $1/f^2$ VCO 留下的 ICI 收敛:
  $\sigma^2=\int_0^\infty \tfrac{k_2}{f^2}[1-\mathrm{sinc}^2(fT)]\,df=\tfrac{\pi^2}{3}k_2T$
  (被积函数在低频端 → $(\pi T)^2/3$,符号内随机游走走不远,均值吸收大半)。
  它随符号长度线性:3.2 µs 地板 −42.9 dB、12.8 µs −36.8 dB(k₂ 来自
  −116.1 dBc/Hz @ 1 MHz)。11ac 最优 −43.7 dB → 地板只高 **0.9 dB**(左侧平坦);
  11ax 最优 −41.9 dB → 地板高 **5.0 dB**(尖谷)。机理:环路收窄后 VCO 噪声进入
  $f_{bw}$–$f_x$(251 kHz),11ac 的 $f_c=138$ kHz 把这段大半吃掉,11ax 的
  $f_c=35$ kHz 吃不到。
- **对 PLL 的含义**:同一颗 PLL 在 11ax 上的环路带宽容差比 11ac 严得多(±0.5 dB
  对应 180–450 kHz vs 10–200 kHz);谷值由 $k_2$ 决定,环路调不下去;11ac 的
  "靠 CPE 收窄环路压参考噪声"经验不能搬到 11ax。这也是 0.7.8 记的"11ax 上
  jitter 最优 = EVM 最优、11ac 上不然"的机理。
- **改动**:`impairments.phase_noise.free_vco_ici_floor(k2, t_fft, n_lo)` 新增
  (公开接口新增,向后兼容);PLL 页画出该地板并标注"高于最优多少 dB",新增
  指标 `vco_floor_post_cpe_db`,页内文字说明左侧饱和;`tools/build_pn_cpe_note.py`
  生成第二篇 `docs/pn_cpe_note_loop_bandwidth.{pdf,png}`(闭式曲线 + 时域
  8 帧圆点 + 两条地板,数字全部由库现算)。
- **守卫**:地板 == 开环 type-II(平台/底关掉)的数值积分(6% 内,实测
  −42.87 vs −42.92 / −36.85 vs −37.04 dB)、两制式恰 4×、双 LO 2×;spec 层断言
  地板高于配置 ② 闭式 3–12 dB 且值为 −36.85 ± 0.3。变异(π²/3 → π²/2)已红。
- **两端**:`golden.json` 重生成(新增一指标);金标 job 重新 dispatch:**run #56
  三 job 全绿**,解释版与编译版端上均 5 run / 0 failed / 0 skipped,金标按
  19 指标 / 4 页通过,编译版 wheel 打 0.7.13。

## 0.7.12 — 2026-09-05

### 文档:英文技术短文《Why CPE removal buys less on 11ax/be than on 11ac/n》

- **需求**:用户要"图示 + 公式"解释为何去 CPE 在 11ax 收益更小,再要英文版,
  再要求渲染成 PDF(聊天里公式显示不正常),最后要求归档进 `docs/`。
- **内容(3 页 A4)**:① CPE 是逐符号减一个均值相位,$J_0\approx e^{j\bar\varphi}$,
  余下全是 ICI;② 符号均值 = 矩形窗平均,频域权重 $\mathrm{sinc}^2(fT)$,交接点
  $0.443/T$ = 138 kHz(3.2 µs)/ 35 kHz(12.8 µs);③ 出货谱平台 −104.1 dBc/Hz
  恰在两交接点之间,平台近似 $S_0/(2T)$ 给 29.6% / 7.4%,精确积分 27.8% / 5.5%,
  CPE 买回 1.41 / 0.24 dB;三联图(时域阶梯、权重叠 L(f)、累积去除比例)。
- **做法**:`tools/build_pn_cpe_note.py` 用 matplotlib mathtext 排版(容器无
  LaTeX),**数字由脚本调用 `cpe_partition()` 现算**,不是抄进去的,不会与库
  漂移;产物 `docs/pn_cpe_note_11ac_vs_11ax.{pdf,png}`。逐页栅格化核过无重叠。
- 只动 `docs/`、`tools/` 与 README;不改随 APK 出货的代码,不重跑金标。

## 0.7.11 — 2026-09-05

### `pn_cpe_study` 第四页文字更正:32 帧并不能"钉死"两制式差值

- **起因**:用户要求 40 MHz 用 32 帧重跑。本制式 11ax 读 −41.77 dB(与此前
  32 帧一致),另一制式 11ac 读 −43.62 dB,差 1.86 dB——而此前同样 32 帧、
  另一随机流读的是 −43.18 / 1.4 dB。于是量了散布:6 个种子 × 32 帧,配置 ④
  11ac 均值 −43.47、1σ 0.15 dB;11ax 均值 −42.06、1σ 0.12 dB;8 帧时 1σ
  0.31 / 0.20 dB;配置 ② 仅 0.05 dB——散布几乎全来自 LTF 冻结误差,它按帧
  不按符号平均。
- **结论(总体均值)**:40 MHz 单 LO 下 11ax 比 11ac 差 **1.4 ± 0.2 dB**
  (32 帧,1σ);0.7.10 页内那次 1.86 dB 是约 2σ 的一次实现,1.4 dB 的口径
  不变,但**"32 帧即可钉死"是我写过头了**——32 帧把差值的 1σ 从 0.4 压到
  0.2 dB,不是压到零。
- **改动**:页 (d) 文字改为报告**实测**散布律(8 帧 0.3 dB、32 帧 0.15 dB
  每根柱,差值 ≈ 0.42/√(帧数/8) dB rms)并注明配置 ② 的 0.05 dB 作对照;
  不再声称"settle"。金标指标未变(文字不进金标);按规则重新 dispatch:
  **run #53 三 job 全绿**,两种形态端上均 5 run / 0 failed / 0 skipped,编译版
  wheel 打 0.7.11。
