...
| AGC 动态 | 空闲档位 → 攻击延迟后切档,VGA 增益与 DC 环各按时间常数指数建立(`RxParams.agc_dynamics`,默认关);帧可带 8 µs L-STF |

**模型边界(2026-09 体检核实,不是疏漏而是范围)**:**交付面的**所有 EVM 都是
**传导模式**数——TX 到 RX 之间只有 AWGN 与单位环回。0.7.30 起有一个**只在链路层**的
色散信道研究(`wifitrx.link.channel`,指数功率时延剖面的抽头延迟线),它**进不了
交付面**:信道住在 `link/` 下,而分层守卫禁止 `cal` 与 `chain` import `link`,
所以这是结构上的保证而不是靠自觉。有效音
默认仍是无空音的连续对称块(标准导频按序号映射进去);0.7.31 起
`OFDMConfig.tone_plan="standard"` 可切到本数字学的**真实计划**(DC 间隙与段间空音
是真空洞,导频直接落在标准音索引上),`tone_plan.punctured` 可按 20 MHz 子信道打孔。
**OFDMA 的 RU 划分仍未建模**,打孔计划也没有自己的导频集(标准另有定义),所以对
打孔计划取导频会直接报错而不是猜;AGC 的建立动态(攻击延迟、VGA 与 DC 环时间常数)0.7.25 起可开,默认关,
且只进研究页不进交付面;交付面不含 MIMO 链间残差。要做链路级(PER)或空口验证,请把 `cal_state.json` 的残差按 `apply` 配方
注入你们自己的链路仿真——那正是重放闭环存在的原因。
