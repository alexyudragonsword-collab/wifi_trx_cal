...
**GUI 工作台**(`python app/main.py`)三个页签:Analyses(十一个分析,含基带噪声扫描、LO 相噪 vs CPE 去除研究、AGC 建立动态、传播信道与子带位置研究)、
...
输出三个数:**解释 / 实测 / 未解释**。容差带由 10-seed 扫描实测确定,
两个视图均为对称 ±2.0(TX 均值 −0.45/σ 0.51;RX 均值 +0.42/σ 0.45)。
