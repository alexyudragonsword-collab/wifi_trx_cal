...
from wifitrx.cal.base import (cal_state_readme, check_schema, load_cal_state,
                              save_cal_state)
...
def _run(bw, qam, with_dpd, seed=3, path=LoopbackPath(atten_db=40.0, delay_ns=6.0)):
    ...
    results = run_full_cal(tx, rx, cfg, path, with_dpd=with_dpd)
...
# ------------------------------------------------ frozen file and schema
FROZEN = Path(__file__).resolve().parent / "data" / "cal_state_v1.json"


def test_frozen_v1_file_still_loads_inspects_and_closes():
    """tests/data/cal_state_v1.json was written by the 0.7.21 writer
    (schema 1.1, 40 MHz / 256-QAM / DPD, seed 3) and is never
    regenerated by the suite: every other test here saves a fresh file
    with the current writer, so a reader that silently stopped
    understanding an older file — a renamed residual key, a dropped
    conditions field, a stricter format check — would pass them all.
    This one reads the committed file with today's library: it must
    load, inspect without errors, replay to closure in both views, and
    every key it ships must still be in this library's RESIDUAL_SPEC
    (the table is the consumer's contract; renaming a key orphans
    every delivered file)."""
    tx_state, rx_state = load_cal_state(FROZEN)
    assert tx_state and rx_state
    doc = json.loads(FROZEN.read_text())
    assert check_schema(doc) == "1.1"
    errors = [f for f in inspect_cal_state(doc) if f["severity"] == "error"]
    assert not errors, errors
    assert set(doc["residuals"]["values"]) <= set(RESIDUAL_SPEC)
    for view in ("tx", "rx"):
        result = replay(FROZEN, view=view)
        assert result.verdict == "consistent", result.summary()


def test_schema_minor_is_tolerated_and_major_is_refused(tmp_path):
    """A 1.0 file (no schema_version — everything before 0.7.21) reads
    with an informational finding; a 2.x file is refused by the loader
    and flagged as an error by the inspector."""
    doc = json.loads(FROZEN.read_text())
    doc.pop("schema_version")
    old = tmp_path / "old.json"
    old.write_text(json.dumps(doc))
    assert check_schema(doc) == "1.0"
    load_cal_state(old)
    infos = [f for f in inspect_cal_state(doc) if "schema 1.0" in f["message"]]
    assert infos and infos[0]["severity"] == "info"
    doc["schema_version"] = "2.0"
    new = tmp_path / "new.json"
    new.write_text(json.dumps(doc))
    with pytest.raises(ValueError, match="not a 1.x"):
        load_cal_state(new)
    assert any(f["severity"] == "error" and "schema 2.0" in f["message"]
               for f in inspect_cal_state(doc))


@pytest.mark.slow
@pytest.mark.parametrize("bw, qam, seed", [(40e6, 256, 11), (40e6, 256, 29),
                                           (320e6, 4096, 3), (320e6, 4096, 17)])
def test_closure_holds_across_bandwidth_qam_and_seed(bw, qam, seed, tmp_path):
    """The closure guard above runs one configuration; a replay recipe
    that only closes at 40 MHz / 256-QAM / seed 3 would pass it.  Every
    step — DPD, IIP2, AGC, group delay included — runs again here on
    other process draws and at the flagship numerology, and the file
    must still inspect clean and close in both views.  The loopback path
    is the bandwidth-dependent cal-coupler design point (path=None): a
    fixed 40 dB starves rx_iq of observation SNR at 320 MHz (IRR 47.1 dB
    against the 50 dB spec on seeds 3 and 17), which is a margin
    finding about the coupler, not about closure."""
    cfg, tx, rx, results = _run(bw, qam, with_dpd=True, seed=seed, path=None)
    out = tmp_path / "cal_state.json"
    save_cal_state(out, tx.correction_state(), rx.correction_state(),
                   results, fs_hz=cfg.sample_rate_hz,
                   conditions=run_conditions(cfg, tx, rx, with_dpd=True))
    doc = json.loads(out.read_text())
    assert not [f for f in inspect_cal_state(doc) if f["severity"] == "error"]
    for view in ("tx", "rx"):
        result = replay(out, view=view)
        assert result.verdict == "consistent", (bw, qam, seed, result.summary())
