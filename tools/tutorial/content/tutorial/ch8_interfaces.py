        ...
        Section(
            ...
            body=(
                T("<code>python app/main.py</code>,三个页签。"
                  "<b>Analyses</b>:八个分析(参数表单由声明式 spec 生成)"
                  ...
                  "漂移跟踪、阻塞退敏、LO 相噪 vs CPE 去除(相噪唯一损伤的四种"
                  "测量配置直读——不去 CPE / genie CPE / LTF 信道估计 / 导频 CPE,"
                  "配置 1/2 叠加闭式解对拍;谱按 1 − sinc²(f·T_FFT) 拆成 CPE 可去除"
                  "与 ICI 两部分;type-II PLL 环路带宽扫描对照 jitter 最优与去 CPE 后"
                  "EVM 最优;两制式并排的误差功率堆叠图;残余 CFO 扫描——不跟踪的"
                  "配置糊掉、genie CPE 留下符号内斜坡、LTF + 导频捕获后不变,即跟踪"
                  "的真正价值)、杂散规划。"
                  ...
                  "<b>Analyses</b>: eight analyses (forms generated from "
                  ...
                  "blocker desense, LO phase noise vs CPE removal (four "
                  "measurement configurations read directly with phase "
                  "noise as the only impairment — no CPE / genie CPE / "
                  "LTF channel estimate / pilot CPE — with the closed "
                  "forms overlaid on configs 1 and 2; the LO profile split "
                  "by 1 − sinc²(f·T_FFT) into CPE-removable and ICI parts; "
                  "a type-II PLL loop-bandwidth sweep comparing the jitter "
                  "optimum with the post-CPE EVM optimum; the two standards side "
                  "by side as stacked error power; a residual-CFO sweep — the untracked "
                  "config smears, genie CPE keeps the in-symbol ramp, LTF + pilot "
                  "acquisition is unaffected: the tracking's real value) and spur "
                  "planning. "
