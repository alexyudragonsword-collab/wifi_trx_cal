    ...
    sections=(
        ...
        Section(
            ...
            body=(
                ...
                T("corner 比例全带宽统一:TX 1.3×BW/2 为 DPD 频谱留带宽"
                  "(洞察②),RX 1.12× 做信道选择。20 MHz 的单 LPF ISI "
                  "地板实测 −55(1.3×)/−50 dB(1.12×),远低于整链 "
                  "~−42 dB 的实际水平;窄带模式不要为 EVM 放宽 corner——"
                  "放宽换来的余量没人消费,却牺牲邻道的模拟抑制(洞察⑤;"
                  "测这个地板还揪出过两层时延补偿伪影,见洞察⑥)。库里 "
                  "<code>recommended_lpf_corner_hz()</code> 封装了这条"
                  "策略。",
                  "The corner ratios are uniform across bandwidths: TX "
                  "1.3×BW/2 preserves the DPD spectrum (insight ②), RX "
                  "1.12× performs channel selection. The measured 20 MHz "
                  "single-LPF ISI floors are −55 (1.3×) / −50 dB "
                  "(1.12×), far below the chain's actual ~−42 dB level; "
                  "narrow modes should NOT relax the corner for EVM — "
                  "the margin bought goes unconsumed while analog "
                  "adjacent-channel rejection is sacrificed (insight ⑤; "
        ...
        Section(
            ...
            body=(
                T("<b>LNA/AGC</b>:八档前端增益态,每档有各自的增益/NF/IIP3/"
                  "切换门限(门限按噪声-IM3 平衡解出);热噪声按当前档的 NF 在"
                  "输入端注入。<b>三阶非线性</b>"
                  ...
                  "偏差(SCO)同源(同一晶振 ppm)。",
                  "<b>LNA/AGC</b>: eight front-end gain states, each with "
                  "its own gain/NF/IIP3/handover threshold (thresholds "
                  "solved from the noise/IM3 balance); thermal noise "
                  ...
                  "one crystal ppm."),
                T("<b>模拟基带(LPF+VGA+ADC 驱动)</b>可以显式建模,但默认关闭:"
                  "上面那张档位表的 NF/IIP3 是<b>级联总值</b>,基带贡献已经含在里面,"
                  "再叠一层就是重复计算(要打开先用 "
                  "<code>link.budget.deembed_states</code> 把总值拆成 RF-only 档位)。"
                  "它的口径按电路给:<b>输入参考噪声电压密度</b>(V/√Hz,默认 "
                  "6 nV/√Hz)与<b>输出摆幅</b>(Vpp,默认 1.0),而不是 NF/IIP3——"
                  "NF 只作为级联算式里的派生量出现。位置也不对称:噪声注在 LPF "
                  "<b>之前</b>(信道滤波器要整形它),压缩施加在 VGA <b>之后</b>"
                  "(输出参考的天花板)。这个不对称正是要点——AGC 把 VGA 输出电平"
                  "钉死,所以输出参考的压缩<b>躲不开任何增益档</b>,它的 EVM 代价与"
                  "天线功率无关,唯一的缓解手段是加大 ADC backoff(见第 9 章)。",
                  "The <b>analog baseband (LPF + VGA + ADC driver)</b> can "
                  "be modelled explicitly, but is off by default: the "
                  "NF/IIP3 in the ladder above are <b>cascade totals</b> "
                  "that already contain the baseband's share, so adding a "
                  "stage on top double-counts it (switching it on requires "
                  "<code>link.budget.deembed_states</code> to split the "
                  "totals into an RF-only ladder). It is specified the way "
                  "a circuit is: an <b>input-referred noise voltage "
                  "density</b> (V/√Hz, default 6 nV/√Hz) and an <b>output "
                  "swing</b> (Vpp, default 1.0) — not NF/IIP3, which appear "
                  "only as derived quantities inside the cascade maths. The "
                  "placement is deliberately asymmetric: noise enters "
                  "<b>before</b> the LPF (the channel filter shapes it), "
                  "compression is applied <b>after</b> the VGA (an "
                  "output-referred ceiling). That asymmetry is the point — "
                  "the AGC servos the VGA <i>output</i> level, so an "
                  "output-referred ceiling is the one impairment <b>no gain "
                  "state escapes</b>: its EVM cost is independent of "
                  "antenna power, and the only relief is more ADC backoff "
                  "(see chapter 9)."),
