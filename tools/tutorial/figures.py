...
def depgraph_svg(ctx) -> str:
    ...
    Delegates to the library so the GUI's Reference tab and this figure
    are the same drawing with the same edge numbering (ctx is unused —
    the graph is derived from the dependency table, not from a run).
    """
