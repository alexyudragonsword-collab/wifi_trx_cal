...
- **modem 口径只差两项估计噪声(≈1.9 dB)"只对白噪声成立**:校准后的链上原始 LTF
  ...
  **更正/补充(0.7.30,实测)**:这个 9 音是**传导模式下**的选择。在色散信道上
  平滑从纯降方差变成偏置-方差权衡,最优跨度随相干带宽 1/(2πτ) 移动:40 MHz /
  30 dB 带内 SNR 下,9 音相对同一扫描的最优值在 rms 时延 ≤ 50 ns 时只亏 ≤ 0.16 dB,
  到 150 ns 亏 6.07 dB(9 音 = 703 kHz,而 Bc 只有 1.06 MHz)。交付默认不变,
  但它的适用前提要写明。见 `cairn/传播信道与估计.md` 与 CHANGELOG 0.7.30。
