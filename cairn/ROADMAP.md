...
2. ~~体检 P2~~ **已全部收口**(2026-09-11):代码卫生 0.7.23、模型边界声明 0.7.24、AGC 建立动态 0.7.25、传播信道 0.7.30、音计划 0.7.31——见 `docs/backlog_zh.md` B23–B25、B29、B30。已判为不做并记明理由:`tx_iq` 多音合并(体检的热点判断被实测推翻)、`tutorial.html` 改 CI 产物、mypy。**评估后三项已落地**(0.7.32–0.7.33):打孔/RU 的模型导频集与子带位置研究、时变多普勒。**仍未建模**:标准的 RU 音索引表与打孔导频表(外部输入,与 A2/A4 同类)、包内信道跟踪(本模型按最简接收机冻结 LTF 估计)。
