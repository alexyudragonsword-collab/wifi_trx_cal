...
- **任何更新都必须把 Qt 与 Android 两端各自验证完整,缺一不算完成**:
  ...
  - Android 侧:同一套桌面测试里的 `test_android_bridge.py`(桥契约、scipy/numpy 调用面守卫、`native.*`↔Kotlin↔bridge 跨层接线、两端检查器区块一致)**加上**重新 dispatch 金标 job;端上实跑条数、失败数与跳过数由 job 自身的 `on-device tests actually ran` 步骤断言并在日志末尾打印清单——**新增端上测试时必须同步更新该步骤里的期望条数**(是精确值不是下限——下限挡不住"新加一条、旧的一条悄悄不跑"这种总数不变的漂移);
...
- 改动随 APK 出货的代码(`src/wifitrx/`、`app/specs.py`、`app/reference.py`、`app/inspector_data.py`、`android/`)或新增端上测试后,必须重新 dispatch `android.yml` 的金标 job,**且派发时必须显式选中被测提交所在的分支**(默认分支 2026-09-11 起是 `main`,而开发在 `claude/…` 分支上——用默认值会裁决 `main`,且当 `main` 恰好等于开发分支顶端时这次误派发在结论上完全看不出来)——**构建绿不等于端上物理对**,而没跑过的端上守卫等于没有守卫(两次事故的教训:scipy/numpy 跨版本 API,以及守卫自身未验证)。条数核对已由 job 内的断言步骤承担,不再依赖人工翻日志(人工翻日志本身也不可靠:日志接口按大小截断,取不到中段那几行)。
- 变异验证守卫时,先确认**变异真的注入成功**再看结论——替换锚点跨行未匹配会让变异变成空操作,守卫"通过"其实什么都没证明(2026-08-24 实际踩到)。
- **守卫要双向验证:变异 → 必须红,还原 → 必须绿。** 只验前者挡不住"恒红"的守卫——它在变异下照样红,和严格的守卫从结论上看一模一样(0.7.16 金标要求精确 0,0.7.26 陈旧检查把 commit id 留在比对文本里,分别红了八天和三周)。同理,**关键检查不要排在会失败的步骤后面**:`set -e` 下它不执行也不报告,日志里与"不存在"无异。四种失效形态与规程见 `cairn/守卫有效性.md`。
- 守卫扫描源码文本时,匹配**那一行真正干活的代码**,而不是名字或词句——注释里有同名词句、函数定义里有同名标识符,朴素子串检查会匹配到它们,于是删掉调用点守卫照样通过(2026-08-24/25 连踩四次:`Python.start`、`annotation_typing=False`、`_drop_package_initializers`)。
