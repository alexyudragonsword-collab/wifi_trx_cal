...
- 守卫扫描源码文本时,匹配**那一行真正干活的代码**,而不是名字或词句——注释里有同名词句、函数定义里有同名标识符,朴素子串检查会匹配到它们,于是删掉调用点守卫照样通过(2026-08-24/25 连踩四次:`Python.start`、`annotation_typing=False`、`_drop_package_initializers`)。写完守卫必须用变异确认它真的会红。
